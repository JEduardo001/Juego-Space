		...
		for (int i2 = 0; i2 < enemigos.length; i2++) {
			...
			if (balaJugadorX <= enemigos[i2].x + 30 && balaJugadorX >= enemigos[i2].x && balaJugadorY > enemigos[i2].y
				...
				if (i2 == 0) {
					...
					score = score + 100 - 20;
					if (vidaBoss - 1 == 0) {
						JOptionPane.showMessageDialog(null,"Haz ganado!!! Tu puntuacion fue: "+score);
			...
			if(enemigos[66].y+20<limiteY-100) {
				if(enemigos[i].x !=0) {
					...
					if (enemigos[i].y + 20 < limiteY) {
						...
						//Audio
						 try {
						        AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new File("Alien3.wav").getAbsoluteFile());
						        Clip clip = AudioSystem.getClip();
						        clip.open(audioInputStream);
						        clip.start();
						    } catch(UnsupportedAudioFileException | IOException | LineUnavailableException ex) {
...
*/
		...
		if (balaEnemigaX != 0) {
			...
			if (balaEnemigaX <= cordJugador[0][0] + 40 && balaEnemigaX > cordJugador[0][0]
				...
				if(vidaJugador<=0) {
					...
					JOptionPane.showMessageDialog(null,"Haz Perdido!!!  Tu puntuacion fue: "+score );
					gameOver=true;
	...
	public class MyGraphics extends JComponent {
		...
		public void paintComponent(Graphics g) {
			...
			g.setFont(Vidas);
			g.setColor(Color.green);
			g.drawString("VIDAS : " + vidaJugador, 60, 30);
			...
			for (int i = 1; i < 14; i++) {
				...
				if (enemigos[i].x != 0) {
					...
					g.drawImage(enemigo3, enemigos[i].x+10, enemigos[i].y, this);
				}
